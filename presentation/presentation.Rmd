...
```{r data-joins, fig.alt = "Scatterplot of police complaints per capita by district name colored by racial majority. Visualization shows neighborhoods with the most complaints per capita are predominantly Black", echo = FALSE, fig.width = 10, fig.height= 5, out.width = "100%"}
district_complaints <- complaints_accused %>%
  filter(current_unit %in% 1:25) %>%
  group_by(current_unit) %>%
  summarise(n = n()) %>%
...
```{r complaints-perc-leaflet, fig.width = 10, out.width = "150%", fig.alt = "Leaflet Map Showing the complaints per capita. Highest number of complaints per capita are in neighborhoods that are majority Black, and one majority white neighboborhood", message = FALSE, echo = FALSE, warning = FALSE, error = FALSE}
...
```{r proportion-findings-vis-prep, echo = FALSE, warning = FALSE, message = FALSE, include = FALSE}
...
```{r r proportion-findings-vis, message = FALSE, echo = FALSE, warning = FALSE, error = FALSE}
  data1 %>%   
  filter(is.na(District_Name) == FALSE) %>%
...
# Final Finding Summery Statistics

```{r some summary stats, echo = FALSE}

#ask why NC and DIS weren't included originally and if they want the R table or knitted table

findings_table <- total_district_complaints_findings %>%
...
```{r complaints-for-sustained-decisions-prep, echo = FALSE, warning = FALSE, message = FALSE, include = FALSE, out.width = "100%"}
...
```{r missing-complaint-data, message = FALSE, echo = FALSE, warning = FALSE, error = FALSE, fig.alt = "Scatterplot of missing police complaints per capita by district name colored by racial majority. Visualization shows neighborhoods with the most complaints per capita are predominantly Black besides one, which is the Central District, a majority white District", fig.width = 10}
