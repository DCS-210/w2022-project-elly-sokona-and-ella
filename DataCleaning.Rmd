...
```{r}
#just ignore this, I (ella) was trying to see if I could combine the demographic maps so that you could select which one with a button but so far I can't figure it out. 
map <- m %>%
  addPolygons(
    fillOpacity = 1,
    color = "black",
    opacity = 0.7,
    weight = 1,
    fill = ~pal_perc(total_district_complaints_spatial$`complaints_per_capita`)) %>%
  addProviderTiles(total_district_complaints_spatial$`Black%`, group = "Black Percent") %>%
  addProviderTiles(total_district_complaints_spatial$`Latino%`, group = "Latino Percent") %>%
   addProviderTiles(total_district_complaints_spatial$`White%`, group = "White Percent") %>%
  addLayersControl(
    baseGroups = 
      c("Black Percent",
        "Latino Percent",
